...
class Tsubasa {
    constructor() {
        ...
        const { token, chatId } = this.loadTelegramCredentials();
        this.bot = new TelegramBot(token, { polling: true });
        this.chatId = chatId;
    }

    loadTelegramCredentials() {
        const credentialsPath = path.join(__dirname, 'telegram_credentials.txt');
        try {
            const content = fs.readFileSync(credentialsPath, 'utf8');
            const lines = content.split('\n');
            const token = lines[0].trim();  // Assuming the first line is the token
            const chatId = lines[1].trim(); // Assuming the second line is the chat ID
            return { token, chatId };
        } catch (error) {
            console.error("Failed to load Telegram credentials:", error);
            process.exit(1); // Exit if credentials are critical
        }
    }

    /**
     * Hàm thoát các ký tự đặc biệt trong HTML.
     * @param {string} text - Chuỗi văn bản cần thoát ký tự.
     * @returns {string} - Chuỗi văn bản đã được thoát ký tự.
     */
    escapeHtml(text) {
        return text
            .replace(/&/g, "&amp;")
    ...
    sendTelegramMessage(msg) {
        const messageWithTitle = `<b>TSUBASA GAME</b>\n<b>Bot By Manh Hieu Phan</b>\n<pre>${msg}</pre>`;
        this.bot.sendMessage(this.chatId, messageWithTitle, { parse_mode: 'HTML' }).catch(error => {
            console.error("Lỗi khi gửi tin nhắn Telegram:", error.message);
        });
    }

    log(msg, type = 'info', accountInfo = '') {
        const timestamp = new Date().toLocaleTimeString();
        const formattedMsg = `[${timestamp}] - ${accountInfo}\n${msg}`;
        switch (type) {
            case 'success':
                console.log(formattedMsg.green);
                break;
            case 'custom':
                console.log(formattedMsg.magenta);
                break;
            case 'error':
                console.log(formattedMsg.red);
                break;
            case 'warning':
                console.log(formattedMsg.yellow);
                break;
            default:
                console.log(formattedMsg.blue);
        }
        if (msg.includes("Balance:") || msg.includes("Năng lượng:") || msg.includes("Coins per tap:") || msg.includes("Lợi nhuận mỗi giây:") || msg.startsWith("========== Tài khoản")) {
            // Thoát các ký tự đặc biệt trong formattedMsg bằng HTML
            const escapedMsg = this.escapeHtml(formattedMsg);
            // Sử dụng HTML để định dạng tin nhắn
            this.sendTelegramMessage(escapedMsg);
        }
    }
    ...
    /**
     * Hàm chờ đợi trong một khoảng thời gian nhất định (ms).
    ...
    async executeTask(initData, taskId, axiosInstance) {
        ...
        try {
            ...
            this.log(`Lỗi khi làm nhiệm vụ ${taskId}: ${error.message}`, 'error');
            return false;
        }
    }
    ...
    async checkTaskAchievement(initData, taskId, axiosInstance) {
        ...
        try {
            ...
            this.log(`Lỗi rồi ${taskId}: ${error.message}`, 'error');
            return { success: false };
        }
    }
    ...
    /**
     * Xử lý một lần cho một tài khoản.
     * @param {string} initData - Dữ liệu khởi tạo cho tài khoản.
     * @param {number} index - Chỉ số của tài khoản (để phân biệt trong logging).
     */
    async processAccountOnce(initData, index) {
        ...
        try {
            ...
            this.log(`Lỗi kiểm tra IP proxy: ${error.message}`, 'warning', `Tài khoản ${index + 1} | ${firstName}`);
            // Bạn có thể quyết định bỏ qua tài khoản này hoặc tiếp tục mà không sử dụng proxy
        }
        
        const accountInfo = `Tài khoản ${index + 1} | ${firstName} | ip: ${proxyIP}`;
        this.log(`========== ${accountInfo} ==========` , 'custom', accountInfo);
        ...
        try {
            ...
            if (startResult.success) {
                if (startResult.total_coins !== undefined) {
                    this.log(`Năng lượng: ${startResult.energy}/${startResult.max_energy}\nBalance: ${startResult.total_coins}\nCoins per tap: ${startResult.coins_per_tap}\nLợi nhuận mỗi giây: ${startResult.profit_per_second}`, 'info', accountInfo);
                }
                    ...
                    for (const task of startResult.tasks) {
                        ...
                        if (executeResult) {
                            ...
                            if (achievementResult.success) {
                                this.log(`Làm nhiệm vụ ${achievementResult.title} thành công | phần thưởng ${achievementResult.reward}`, 'success', accountInfo);
                            }
                        }
                    }
                } else {
                    this.log(`Không có nhiệm vụ nào khả dụng.`, 'warning', accountInfo);
                }

                if (startResult.energy !== undefined) {
                    const tapResult = await this.callTapAPI(initData, startResult.energy, axiosInstance);
                    if (tapResult.success) {
                        this.log(`Tap thành công | Năng lượng còn ${tapResult.energy}/${tapResult.max_energy} | Balance : ${tapResult.total_coins}`, 'success', accountInfo);
                    } else {
                        this.log(tapResult.error, 'error', accountInfo);
                    }
                }

                const dailyRewardResult = await this.callDailyRewardAPI(initData, axiosInstance);
                this.log(dailyRewardResult.message, dailyRewardResult.success ? 'success' : 'warning', accountInfo);
                const updatedTotalCoins = await this.levelUpCards(initData, startResult.total_coins, axiosInstance);
                this.log(`Đã nâng cấp hết các thẻ đủ điều kiện | Balance: ${updatedTotalCoins}`, 'success', accountInfo);
            } else {
                this.log(startResult.error, 'error', accountInfo);
            }
        } catch (error) {
            this.log(`Lỗi xử lý: ${error.message}`, 'error', accountInfo);
        }
    }
    ...
    async main() {
        ...
        while (true) {
            this.log('========== Bắt đầu vòng lặp xử lý các tài khoản ==========', 'custom');
            // Tạo một mảng các Promises để xử lý đồng thời tất cả các tài khoản
            const accountPromises = data.map((initData, i) => this.processAccountOnce(initData, i));
            // Chờ tất cả các tài khoản xử lý xong
            await Promise.all(accountPromises);
            this.log('========== Hoàn thành vòng lặp xử lý các tài khoản ==========', 'custom');
            // Chờ 600 giây (10 phút) trước khi bắt đầu vòng lặp tiếp theo
            this.log('===== Chờ 600 giây (10 phút) trước khi bắt đầu vòng lặp tiếp theo =====', 'warning');
